...
const Board = ({data, addBoard, board, addCard, mouseDown, mouseUp}) => {
  ...
  return (
    <div className={styles.board} onMouseUp={() => mouseUp(board)}> 
      {addBoardButtonView}
      {data.map((card, i) => {
      	if (i === 0) return;
      	return <Card mouseDown={mouseDown} board={board} card={i} data={card}/>
      })}
